...
const Sidebar = () => {
  ...
  return (
    <div className={clsx("fixed inset-0 w-full h-full z-[13] font-sans transition-opacity duration-300", isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none")}>
      ...
      {/* Sidebar */}
      <div className={clsx("relative z-[15] w-full h-full md:max-w-[364px] bg-white overflow-hidden transition-transform duration-300", isOpen ? "translate-x-0" : "-translate-x-full")}>
        {/* Main Sidebar */}
